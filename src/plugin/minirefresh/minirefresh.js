...
(function (global, factory) {
  ...
  function isObject(object) {
    var classType = Object.prototype.toString
      .call(object)
      .match(/^\[object\s(.*)\]$/)[1];
  ...
  function extend() {
    ...
    var _loop = function _loop() {
      // source的拓展
      var source =
        _arguments.length <= sourceIndex ? undefined : _arguments[sourceIndex];
    ...
    _createClass$1(Scroll, [
      {
        ...
        value: function _init() {
          ...
          setTimeout(function () {
            if (
              _this.options.down &&
              _this.options.down.isAuto &&
              !_this.options.down.isLock
            ) {
              // 满足自动下拉,需要判断是否需要动画（仅仅是首次）
              if (_this.options.down.isAllowAutoLoading) {
                _this.triggerDownLoading();
              } else {
                _this.events[EVENT_DOWN_LOADING] &&
                  _this.events[EVENT_DOWN_LOADING](true);
              }
            } else if (
              _this.options.up &&
              _this.options.up.isAuto &&
              !_this.options.up.isLock
            ) {
      ...
      },
      {
        ...
        value: function translateContentWrap(y, duration) {
          ...
          wrap.style.webkitTransform =
            "translate(0px, " + translateY + "px) translateZ(0px)";
      ...
      },
      {
        ...
        value: function _scrollWrapAnimation() {
          this.scrollWrap.webkitTransitionTimingFunction =
            "cubic-bezier(0.1, 0.57, 0.1, 1)";
          this.scrollWrap.transitionTimingFunction =
            "cubic-bezier(0.1, 0.57, 0.1, 1)";
        },
      },
      {
        key: "touchstartEvent",
        value: function touchstartEvent(e) {
      ...
      },
      {
        ...
        value: function _initPullDown() {
          ...
          this.touchstartEvent_bind = this.touchstartEvent.bind(this);
          this.touchendEvent_bind = this.touchendEvent.bind(this);
          this.touchmoveEvent_bind = this.touchmoveEvent.bind(this);

          scrollWrap.addEventListener("touchstart", this.touchstartEvent_bind);
          scrollWrap.addEventListener("mousedown", this.touchstartEvent_bind);

          scrollWrap.addEventListener("touchend", this.touchendEvent_bind);
          scrollWrap.addEventListener("mouseup", this.touchendEvent_bind);
          scrollWrap.addEventListener("mouseleave", this.touchendEvent_bind);

          scrollWrap.addEventListener("touchmove", this.touchmoveEvent_bind);
          scrollWrap.addEventListener("mousemove", this.touchmoveEvent_bind);
        },
      },
      {
        key: "_scroll",
      ...
      },
      {
        ...
        value: function _initPullUp() {
          ...
          var targetScrollDom =
            scrollWrap === document.body ? window : scrollWrap;
          this._scroll_bind = this._scroll.bind(this);
          targetScrollDom.addEventListener("scroll", this._scroll_bind);
        },
      },
      {
        key: "_loadFull",
      ...
      },
      {
        ...
        value: function triggerDownLoading() {
          ...
          if (
            ...
            this.translateContentWrap(
              options.down.offset,
              options.down.bounceTime,
            );
      ...
      },
      {
        ...
        value: function resetUpLoading() {
          ...
          this.events[EVENT_RESET_UP_LOADING] &&
            this.events[EVENT_RESET_UP_LOADING]();
        },
  ...
  var Core = (function () {
    ...
    function Core(options) {
      ...
      this.scrollWrap = this.options.isUseBodyScroll
        ? document.body
        : this.container;
      ...
      // 初始化的hook
      this._initHook &&
        this._initHook(this.options.down.isLock, this.options.up.isLock);
    ...
    _createClass(Core, [
      {
        ...
        value: function _initEvent() {
          ...
          this.scroller.on("upLoading", function () {
            _this._upLoaingHook &&
              _this._upLoaingHook(_this.options.up.isShowUpLoading);
            options.up.callback &&
              options.up.callback(_this.options.up.isShowUpLoading);
          });
          ...
          this.scroller.hook(
            "beforeDownLoading",
            function (downHight, downOffset) {
              return (
                !_this._beforeDownLoadingHook ||
                _this._beforeDownLoadingHook(downHight, downOffset)
              );
            },
          );
        },
      ...
      },
      {
        ...
        value: function _endDownLoading(isSuccess, successTips) {
          ...
          if (this.scroller.downLoading) {
            ...
            if (successAnim) {
              // 如果有成功动画
              this._downLoaingSuccessHook &&
                this._downLoaingSuccessHook(isSuccess, successTips);
            } else {
              // 默认为没有成功动画
              successAnimTime = 0;
            }
      ...
      },
      {
        ...
        value: function unload() {
          ...
          targetScrollDom.removeEventListener(
            "scroll",
            this.scroller._scroll_bind,
          );

          scrollWrap.removeEventListener(
            "touchstart",
            this.scroller.touchstartEvent_bind,
          );
          scrollWrap.removeEventListener(
            "mousedown",
            this.scroller.touchstartEvent_bind,
          ...
          scrollWrap.removeEventListener(
            "touchend",
            this.scroller.touchendEvent_bind,
          );
          scrollWrap.removeEventListener(
            "mouseup",
            this.scroller.touchendEvent_bind,
          ...
          scrollWrap.removeEventListener(
            "touchmove",
            this.scroller.touchmoveEvent_bind,
          );
          scrollWrap.removeEventListener(
            "mousemove",
            this.scroller.touchmoveEvent_bind,
